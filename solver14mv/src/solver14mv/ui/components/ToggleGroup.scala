...
object ToggleGroup {
  ...
  def apply(
        ...
        ToggleGroupPrimitive
          ...
          .apply(
            js.Dynamic.literal(
              ...
              className = cn(s"$classNameBase $className"),
              multiple = multiple,
    ...
    modSeq(
      globalPortalOne(Signal.combine(valueVar, className).map {
        case (value, className) => ref => render(value, className, ref)
      }),
