...
object Solver {
  ...
  def solve(
    ...
    val constraint = Constraint.debug(input.constraint)
    val m = clues.length
    val n = clues.lift(0).map(_.length).getOrElse(0)

    given z3.Context = ctx
    Solver14MV[IO](m, n, constraint).use { solver =>
      ...
      val check = solver.check.flatMap {
        case CheckResult.Sat => solver.getSafeCells
        case CheckResult.Unknown =>
          IO.println("unknown") *> Seq.empty.pure[IO]
        case CheckResult.Unsat =>
          IO.println("unsat") *> Seq.empty.pure[IO]
      }
      addClues *> check
    }
  }
